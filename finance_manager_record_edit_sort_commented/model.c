...
/*
 * 修改指定位置的一条记录。
 * 参数 index 使用数组下标，从 0 开始。
 *
 * 步骤：
 * 1. 检查 store、record 是否为空。
 * 2. 检查 index 是否在有效范围内。
 * 3. 用新的 Record 覆盖原来的记录。
 */
int store_update(RecordStore *store, int index, const Record *record) {
    if (store == NULL || record == NULL) {
        return 0;
    }
 ...
 * 1. 检查 store 是否为空。
 * 2. 检查 index 是否在有效范围内。
 * 3. 从 index 后面开始，把每条记录向前移动一位。
 * 4. count 减 1，表示记录数量减少。
 *
 * 注意：这里不调用 realloc 缩小容量。
 * 因为删除记录后保留原容量，后续再添加记录时可以继续使用这块内存。
 */
int store_delete(RecordStore *store, int index) {
    int i;
